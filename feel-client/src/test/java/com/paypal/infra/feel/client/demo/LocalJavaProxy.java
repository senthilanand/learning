...
public class LocalJavaProxy extends Thread
{
	 ...
	 */
	public LocalJavaProxy (int port)
	{
		thisPort = port;
	}
	
	public LocalJavaProxy (int port, String proxyServer, int proxyPort)
	{
		thisPort = port;
	...
	public LocalJavaProxy (int port, String proxyServer, int proxyPort, int timeout)
	{
		thisPort = port;
		fwdServer = proxyServer;
		fwdPort = proxyPort;
	...
	public void run()
	{
		try {
			...
			if (debugLevel > 0)
				debugOut.println("jProxy Thread error: " + e + "  " + Thread.currentThread());
		}
		//closeSocket();
	}
